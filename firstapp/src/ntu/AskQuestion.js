...
const AskQuestion = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post('http://127.0.0.1:8000/askntu', { question });
      const botChat = { type: 'one', message: response.data.response };
  ...
  return (
    <div className="main" id = "zohaib">
      ...
      <div className='chatfooter' id = "huzaifa">
        <div className="inp" id ="saad">
          <input type="text" placeholder="Ask About NTU" value={question} onChange={handleInputChange} disabled={isLoading}/>
