...
import './agridesign.css'; // Import your CSS file
...
const AskQuestionagri = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post('http://127.0.0.1:8000/askagri', { question });
      const botChat = { type: 'different', message: response.data.response };
  ...
  return (
    <div className="main" id="agrizohaib">
      <div className="chats">
        {chats.map((chat, index) => (
          <div key={index} className={`chat ${chat.type}`}>
            <img src={chat.type === 'user' ? usericon : gptimglogo} alt="" />
            <h3 className="txt">{chat.message}</h3>
          </div>
        ))}
        <div ref={chatsEndRef} />
      </div>
      <div className="chatfooter" id="agrihuzaifa">
        <div className="inp" id="agrisaad">
          <input
            type="text"
            placeholder="Ask About Agri"
            value={question}
...
export default AskQuestionagri;
